...
@app.route('/cart')
@login_required  # Require login to view cart
def cart():
    # Ensure user has an active order
    if 'order_id' not in session:
        get_or_create_active_order(current_user.id)
    ...
    try:
        with conn.cursor() as cursor:
            # Join order_item with product to get product details
            cursor.execute("""
...
def checkout():
    ...
    if request.method == 'POST' and request.form.get('order_confirmed') == 'true':
        ...
        try:
            with conn.cursor() as cursor:
                # Update all order items in this order to be checked out
                cursor.execute(
                    "UPDATE order_item SET checkedout = 1 WHERE order_id = %s",
                    (order_id,)
                )
                
                # Mark the order as completed with updated timestamp
                cursor.execute(
                ...
                # Create a new active order for future purchases
    ...
    # Get cart items for the checkout page
    conn = get_db_connection()
    cart_items = []
    total_amount = 0
    ...
    try:
        with conn.cursor() as cursor:
            # Join order_item with product to get product details
            cursor.execute("""
                SELECT order_item.*, product.name, product.description, product.image_url 
                FROM order_item 
                JOIN product ON order_item.product_id = product.idproduct
